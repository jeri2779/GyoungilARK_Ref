...
// 영웅 머리 위 체력바. EnemyHealthBar와 같은 구조(빌보드·보간·표시 여부)지만
// 영웅에는 없는 두 개념(보스 상시표시용 EnemyClass, 은신용 forceHidden)은 뺐다.
public class HeroHealthBar
{
    ...
    public void Tick(float hp, float maxHp, bool isDead)
    {
        if (_slider == null) return;
        ...
        //bool visible = isDead ? !drained : damaged;
        //bool visible = isDead ? !drained;
        bool visible = !isDead;
        if (_shown != visible)
        {
            _shown = visible;
